...
```
There are `r sum(na_value)` NAs in the dataset and all of them falls in _steps_ variable. To be more specific, for each day, it has `r length(interval_level)` records and there are total `r na_freq_table[[2]]` days' records missing.
